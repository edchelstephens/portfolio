...
class CustomManagementCommandMixin(BaseCommand):
    ...
    def write_mapping_to_stdout(self, mapping: Mapping, add_newline=True) -> None:
        """Write mapping to standard output"""
        for key, value in mapping.items():
            self.stdout.write(self.style.SQL_COLTYPE(f"{key} : {value}"))

        if add_newline:
            self.stdout.write("\n")

    def write_iterable_items_to_stdout(
        self, iterables: Iterable, style="SQL_COLTYPE", method=repr
    ) -> None:
        ...
        for item in iterables:
            write_style = (
                ...
                if style == "SQL_KEYWORD"
                else self.style.SQL_COLTYPE
            )
            self.stdout.write(write_style(method(item)))

    def show_stdout_styles(self) -> None:
